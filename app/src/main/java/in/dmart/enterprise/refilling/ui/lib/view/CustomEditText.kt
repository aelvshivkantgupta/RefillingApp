            ...
            setOnKeyListener(OnKeyListener { _, keyCode, event -> // Logger.log(TAG, "inside et_BTEInput.setOnKeyListener of showSearchPopupDialog ");
                ...
                if (event.action == KeyEvent.ACTION_DOWN &&
                    ...
                    if (scanData.isNotEmpty()) {
                        if (key != null) {
                            FireBaseUtil.sendEvent(null, FireBaseUtil.getScanEvent(scanDataListener.javaClass.simpleName, Constant.SCAN_QR), scanDataListener.javaClass.simpleName, FireBaseUtil.EVENT_TYPE_SCAN, scanData)
                            ...
                            FireBaseUtil.sendEvent(null, FireBaseUtil.getScanEvent(scanDataListener.javaClass.simpleName, Constant.SCAN_BAR), scanDataListener.javaClass.simpleName, FireBaseUtil.EVENT_TYPE_SCAN, scanData)
                            scanDataListener.onScanData(this@CustomEditText, scanData.trim { it <= ' ' })
                        }
                        if (isResetText) {
